...
describe("man command", () => {
  ...
  it("exports invoices to CSV on stdout", async () => {
    ...
    const exitCode = await runCli(
      ["export", "--address", validAddress(), "--output", "-"],
      {
        createClient: () => client as any,
        loadConfig: () => TEST_CONFIG,
        stderr: createMemoryStream(),
    ...
    expect(exitCode).toBe(0);
    expect(client.listInvoicesByAddress).toHaveBeenCalledWith(expect.any(String));
    expect(stdout.toString()).toContain("id,freelancer,payer,amount,discountRate,dueDate,status,funder,fundedAt");
  ...
  it("prints protocol-wide analytics from iln stats", async () => {
    const stdout = createMemoryStream();
    vi.spyOn(sdk, "AnalyticsSDK").mockImplementation(
      function MockAnalyticsSDK() {
        return {
...
function createInvoice(overrides: Partial<Invoice> & Pick<Invoice, "id"> & { status?: string }): Invoice {
