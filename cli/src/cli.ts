...
export async function runCli(
  ...
  program
    ...
    .action(async (options: { id: string; interval: string }) => {
      ...
      const print = (invoice: Invoice) => {
        ...
        if (globalOpts.json) {
          stdout.write(JSON.stringify({ ts, invoiceId: invoice.id.toString(), status: invoice.status }) + "\n");
        } else {
  ...
  program
    ...
    .action(async (options: { apiUrl?: string }) => {
      ...
      const baseUrl =
        options.apiUrl ?? (config.network === "mainnet" ? "https://api.iln.network" : "http://localhost:3001");
  ...
  // Reputation command
  const reputationCommand = program.command("reputation").description("Query on-chain reputation scores.");

  reputationCommand
    .command("get [address]")
    .description("Get the reputation score of a Stellar address (defaults to the configured signer).")
    .addHelpText(
      "after",
      [
        "",
  ...
  // Network command
  const networkCommand = program.command("network").description("Manage the active Stellar network.");

  networkCommand
    .command("switch <target>")
    .description("Switch the active network (testnet, mainnet, or standalone) in the local config file.")
    .addHelpText(
      "after",
      [
        "",
        ...
        helpExample("Clears any explicit `rpcUrl` override so the new network's default RPC endpoint applies."),
        "",
        helpSection("See also:"),
